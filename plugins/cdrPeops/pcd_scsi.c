...
/////////////////////////////////////////////////////////
// D8/D4: used by... mmm... good question :) 
/////////////////////////////////////////////////////////
...
DWORD ReadSCSI_D8(BOOL bWait,FRAMEBUF * f)
{
 ...
 sx.SRB_Lun        = iCD_LU;
 sx.SRB_Flags      = SRB_DIR_IN | SRB_EVENT_NOTIFY;
 sx.SRB_BufLen     = f->dwFrameCnt * 2352;
 sx.SRB_BufPointer = &(f->BufData[0]);
 sx.SRB_SenseLen   = SENSE_LEN;
 sx.SRB_CDBLen     = 12;
 sx.SRB_PostProc   = (LPVOID)hEvent;
...
DWORD ReadSCSI_D8_Sub(BOOL bWait,FRAMEBUF * f)
{
 ...
 memcpy(&SubCData[12],&f->BufData[2352],10);
...
{
 DWORD dwStatus;unsigned char cB[20];
 ...
 sx.SRB_BufLen     = 20;//44;
 sx.SRB_BufPointer = cB;
 sx.SRB_CDBLen     = 10;
 sx.SRB_PostProc   = (LPVOID)hEvent;

 memset(&sx.CDBByte[0],0,10);

 sx.CDBByte[0]     = 0x42;
 sx.CDBByte[1]     = (iCD_LU<<5)|2;
 sx.CDBByte[2]     = 0x40;
 sx.CDBByte[3]     = 0x01;
 sx.CDBByte[6]     = 0;
 sx.CDBByte[7]     = 0;
...
DWORD CheckSCSIReadMode(int * iFType,FRAMEBUF * f)
{
 ...
 int iModes[7]={MODE_BE_2,MODE_BE_1,MODE_28_1,MODE_28_2,MODE_D8_1,MODE_D4_1,MODE_D4_2};

 *iFType=-1;

 for(i=0;i<7;i++)                                      // loop avail read modes
  {
   pDeInitFunc = NULL;
   ...
   for(j=0;j<3;j++)                                    // try it 3 times
    {
     ...
     if(dwStatus==SS_COMP)                             // reading was a success, no AA's?
      {
       if(pDeInitFunc) pDeInitFunc();                  // -> do deinit
       *iFType=iModes[i];                              // -> set found mode
       return dwStatus;                                // -> bye
      }
    }
