...
HANDLE          pcd_hReadThread  = NULL;
BOOL            pcd_bThreadEnded = FALSE;
...
void CreateREADBufs(void)
{
 ...
 pCurrReadBuf=pFirstReadBuf=pMainBuffer+12+FRAMEBUFEXTRA;
 ...
 if(iUseCaching>=2)                                    // async/thread mode
  {
   ...
   pAsyncFirstReadBuf[1]=pAsyncBuffer+12+FRAMEBUFEXTRA;

   if(iUseCaching>=3)                                  // thread mode
    {
     DWORD dw;
     pcd_bThreadEnded = FALSE;
     ...
     if(iUseCaching==3)                                // -> create thread
      pcd_hReadThread=CreateThread(NULL,0,READThread,0,0,&dw);
     else
      {
       for(dw=0;dw<MAXQSIZE;dw++) lAddrQ[dw]=0xFFFFFFFF;
       iQPos=0;
       iQLockPos=-1;
       pcd_hReadThread=CreateThread(NULL,0,READThreadEx,0,0,&dw);
      }
    }
  }
 else
 ...
 if(iUseCaching &&                                     // cache block available?
    ...
    addr<=(lLastAddr+MAXCACHEBLOCK))
  {
   pCurrReadBuf=pFirstReadBuf+                         // -> calc data ptr
                 ((addr-lLastAddr)*CDSECTOR);
   if(ppfHead) PCDCheckPPFCache(addr,pCurrReadBuf-12); // -> apply ppf
   return TRUE;                                        // -> done
  }
   ...
   if((addr+MAXCACHEBLOCK)<lMaxAddr)                   // and big read is possible?
    {
     ...
     f->dwBufLen   = (MAXCACHEBLOCK+1)*CDSECTOR;
...
BOOL DoReadAsync(unsigned long addr)
{
 ...
 if(lLastAddr!=0xFFFFFFFF &&
    ...
    addr<=(lLastAddr+MAXCACHEBLOCK))
  {
   ...
   if(ppfHead) PCDCheckPPFCache(addr,pCurrReadBuf-12);
...
DWORD WINAPI READThread(LPVOID lpParameter)
{
 ...
 while(WaitForMultipleObjects(2,hThreadEvent,FALSE,    // wait until event to start (or event to end) get raised
       INFINITE)==WAIT_OBJECT_0)
  {
   ...
   f->dwBufLen    = f->dwFrameCnt*CDSECTOR;
...
{
 if(!pcd_hReadThread) return FALSE;                    // no thread, no fun
   ...
   while(1)
    {
     ...
     if(pReadFunc(TRUE,f)!=SS_COMP)                    // do a blocking (sync) read
      {                                                // mmm... if reading fails a number of times, we don't have a chance to return 'bad' to emu with tread reading... life is hard :)
...
// spinning to much, giving other pc resources no room
...
void GetREADThreadExPtr(void)
{
 ...
 while(1)
  {
   if(pcd_bThreadEnded) return;                        // main emu is already closing (thread is down)? bye
   WaitForSingleObject(hThreadMutex[1],INFINITE);      // wait for data access
   ...
   for(i=0;i<MAXDATACACHE;i++)                         // init all cache slots
    {
     ...
     pDataCacheBuf[i]  = malloc(MAXCDBUFFER-12-FRAMEBUFEXTRA);
    }
  }
}
   ...
   if(addr>=lDataCacheAddr[i] && 
      addr<=(lDataCacheAddr[i]+MAXCACHEBLOCK))
    {
     pCurrReadBuf=pDataCacheBuf[i]+                
                   ((addr-lDataCacheAddr[i])*CDSECTOR);
