...
#ifdef _XBOX
#include "..\..\xbox\glue\sys\undocumented.h"
#endif

/////////////////////////////////////////////////////////

HANDLE hIOCTL=NULL;                                    // global drive file handle
unsigned long ulIOCTLAttr=0;                           // open attribute
...
HANDLE OpenIOCTLFile(/*char cLetter,*/BOOL bAsync)
{
 ...
 if(bAsync) ulIOCTLAttr=FILE_FLAG_OVERLAPPED;
 else       ulIOCTLAttr=0;
 ...
 if(!NT_SUCCESS(error = NtCreateFile(&hF,			//OUT PHANDLE FileHandle,
		            GENERIC_READ|GENERIC_WRITE,     //IN ACCESS_MASK DesiredAccess,
					&attributes,					//IN POBJECT_ATTRIBUTES ObjectAttributes,
					&status,						//OUT PIO_STATUS_BLOCK IoStatusBlock,
					NULL,							//IN PLARGE_INTEGER AllocationSize OPTIONAL,
					ulIOCTLAttr,					//IN ULONG FileAttributes,
					FILE_SHARE_READ,				//IN ULONG ShareAccess,
					OPEN_EXISTING,					//IN ULONG CreateDisposition,
...
DWORD IOCTLSendASPI32Command(LPSRB pSRB)
{
 ...
 if(ulIOCTLAttr==FILE_FLAG_OVERLAPPED)                 // async?
  {
   ovcIOCTL.Internal=0; 
