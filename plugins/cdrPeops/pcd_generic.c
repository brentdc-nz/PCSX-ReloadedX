...
int        iInterfaceMode=2;                           // interface (aspi/ioctrlscsi/ioctrlraw)
...
{
 if(pcd_hReadThread)                                   // thread mode?
...
#else
     SysMessage("(%s)Failure: cannot change the drive speed!\n", libraryName);
#endif
     iUseSpeedLimit=0;
    }
  }
...
#ifndef _XBOX // We don't use aspi
 else                                                  // aspi interface?
  {
