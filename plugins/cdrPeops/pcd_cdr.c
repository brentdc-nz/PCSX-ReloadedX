...
#ifndef _XBOX // Hardcoded as we know what the drive can do on Xbox and best config
 ReadConfig();                                         // read user config
#endif

 PCDBuildPPFCache();                                   // build ppf cache
...
long CALLBACK Plugin_CDRclose(void)
{
 ...
 PCDFreePPFCache();                                       // free ppf cache
...
{
 if(!bIsOpen)  Plugin_CDRopen();                       // not open? funny emu...
...
{
 if(!bIsOpen)   Plugin_CDRopen();                      // usual checks
