...
//iUseScanLines=0; // Not needed on XBox, if we want scanlines we will use a D3D shader
 ...
 bUseFrameSkip = XboxConfigs_GetBool("video.frameskip"); //=FALSE;
 iFrameLimit=2;
 fFrameRate=200.0f;
 iOffscreenDrawing = XboxConfigs_GetInt("video.offscreendrawing");
 bOpaquePass=TRUE;
//bUseAntiAlias=TRUE; // Not used ATM
 iTexQuality=0;
 iUseMask=0;
 iZBufferDepth=0;
 bUseFastMdec = XboxConfigs_GetBool("video.usefastmdec");
 dwCfgFixes=0;
 bUseFixes=FALSE;
 iFrameTexType=1;
 iFrameReadType = XboxConfigs_GetInt("video.framebufferupload"); //=0;
 bUse15bitMdec=FALSE; // Not used ATM
 iShowFPS = XboxConfigs_GetBool("video.showfps");
 bGteAccuracy=0; // Not used ATM
 bKeepRatio = XboxConfigs_GetBool("video.keepaspectratio");
 bForceRatio43=FALSE;
//iScanBlend=0;// Not needed on XBox, if we scanlines we will use a D3D shader
 iVRamSize = XboxConfigs_GetInt("video.vramsize");
 iTexGarbageCollection=1;
//iBlurBuffer=0; // Not needed on XBox, if we want blur we will use a D3D shader
 iHiResTextures = XboxConfigs_GetInt("video.hirestextures");
//iForceVSync=-1; // Done using FGLx
